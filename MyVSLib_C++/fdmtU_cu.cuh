...
void fncFdmtU_cu(fdmt_type_* piarrImage // input image
	, fdmt_type_* d_piarrImage       // on-device auxiliary memory buffer
	, const int IImgrows, const int IImgcols // dimensions of input image 	
	, fdmt_type_* d_piarrState0		// on-device auxiliary memory buffer
	, fdmt_type_* d_piarrState1		// on-device auxiliary memory buffer
	, const int IDeltaT
	, const int I_F
	, const float VAl_dF
	...
	, const  float VAlFmax
	, const int IMaxDT
	, fdmt_type_* u_piarrImOut			// OUTPUT image, dim = IDeltaT x IImgcols
);
...
void kernel_init_fdmt0(fdmt_type_* d_parrImg, const int IImgrows, const int IImgcols
	, const int IDeltaT, fdmt_type_* d_parrOut);
