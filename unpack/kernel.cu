...
//-----------------------------------------------------------------
//INPUT:
// d_parrInput - input matrix, dimentions: nchan x (lenChunk * npol) 
...
int main(int argc, char* argv[])
{
    ...
    const int  lenChunk = 16;
    const int  nchan = 2;
    const int  npol = 4;
    inp_type_* d_parrInput = 0;
    cudaMallocManaged((void**)&d_parrInput, sizeof(inp_type_) * lenChunk * nchan * npol);
    for (int i = 0; i < lenChunk * nchan * npol; ++i)
    {
        d_parrInput[i] = i;
    }
    ...
    cudaMalloc(reinterpret_cast<void**>(&pcmparrRawSignalCur), lenChunk * nchan * npol / 2 * sizeof(cufftComplex));


    const dim3 blockSize = dim3(4, 1, 1);
    const dim3 gridSize = dim3((lenChunk + blockSize.x - 1) / blockSize.x, nchan, 1);
    unpackInput<<< gridSize, blockSize>>>(pcmparrRawSignalCur, d_parrInput, lenChunk
        , nchan, npol);
    using scalar_type1 = float;
    using data_type1 = std::complex<scalar_type1>;
    std::vector<data_type1> data1(lenChunk * nchan * npol / 2, 0);
    cudaMemcpyAsync(data1.data(), pcmparrRawSignalCur,  sizeof(/*data_type1*/cufftComplex) * data1.size(),
