...
int main(int argc,char *argv[])
{
  ...
  for (iblock=0;;iblock++) {
	...
	decimate_and_redigitize<<<gridsize,blocksize>>>(dfbuf,ndec,mchan,mblock,msum,zavg,zstd,3.0,5.0,dcbuf); 
      ...
      int treadsPerBlock = 256;
      size_t sz = treadsPerBlock * sizeof(int);
...
// convolution.
__global__ void unpack_and_padd(char *dbuf0,char *dbuf1,char *dbuf2,char *dbuf3,int nsamp
    ,int nbin,int nfft,int nsub,int noverlap,cufftComplex *cp1,cufftComplex *cp2)
{
