...
//------------------------------------------------------------
__global__
void clean_out_the_trash_kernel(float* d_arr, const int NRows, const int NCols, float* d_buff, const float mean, const float std)
{
...
//------------------------------------------------------------
__global__
void clean_out_the_trash_kernel_v1(float* d_arr, const int NRows, const int NCols, float* d_buff, const float mean, const float std)
{
...
int main()
{
    ...
    for (int irow = 5; irow < 5 + nRows/10; ++irow)
    {
        float valChet = 1.;
        for (int j = 0; j < nCols; ++j)
        {
           // arr[irow * nCols + j] += /*100. +*/ valChet * 4.;
            valChet = -valChet;
        }
   ...
   // h_variance = h_variance/ arraySize - h_mean * h_mean;
    ...
    /////////////////////////////////////////////////////////
    float* d_buff = NULL;
    cudaMallocManaged(&d_buff, (nRows +2) * sizeof(float));
    ...
    {
        // Calculate mean and variance
        cleanInpFDMT_v0(d_arr, nRows, nCols, d_buff);     
      
    }

    end = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
    std::cout << "6 Time of  cleaning : " << duration.count() / ((double)num) << " microseconds" << std::endl;
    cudaMemcpy(&h_mean, &d_buff[nRows], sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(&h_variance, &d_buff[nRows +1], sizeof(float), cudaMemcpyDeviceToHost);
    // Output the mean and variance
    std::cout << "Dispersions Mean: " << h_mean << std::endl;
    std::cout << "Dispersions std: " << h_variance << std::endl;
    std::cout << "------- " << std::endl;
