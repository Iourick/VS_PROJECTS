...
int main(int argc,char *argv[])
{
 ...
 cudaMalloc((void **) &cp1,sizeof(cufftComplex)*nbin*nfft*nsub);
 cudaMalloc((void **) &cp2,sizeof(cufftComplex)*nbin*nfft*nsub);
 ...
 cudaMalloc((void **) &dc,sizeof(cufftComplex)*nbin*nsub*ndm);

  // Allocate device memory for block sums
 cudaMalloc((void **) &bs1,sizeof(float)*mblock*mchan);
 cudaMalloc((void **) &bs2,sizeof(float)*mblock*mchan);

  // Allocate device memory for channel averages and standard deviations
 cudaMalloc((void **) &zavg,sizeof(float)*mchan);
 cudaMalloc((void **) &zstd,sizeof(float)*mchan);
 ...
 cudaMalloc((void **) &dcbuf,sizeof(unsigned char)*msamp*mchan/ndec);
  ...
  for (iblock=0;;iblock++) {
    ...
    startclock=clock();
    for (i=0;i<4;i++)
      cudaMemcpy(dh5buf[i],h5buf[i],sizeof(char)*nread*nsub,cudaMemcpyHostToDevice);
    ...
    // Perform FFTs
    cufftExecC2C(ftc2cf,(cufftComplex *) cp1p,(cufftComplex *) cp1p,CUFFT_FORWARD);
    cufftExecC2C(ftc2cf,(cufftComplex *) cp2p,(cufftComplex *) cp2p,CUFFT_FORWARD);
    ...
    for (idm=0;idm<ndm;idm++) {
      ...
      // Perform FFTs
      cufftExecC2C(ftc2cb,(cufftComplex *) cp1,(cufftComplex *) cp1,CUFFT_INVERSE);
      cufftExecC2C(ftc2cb,(cufftComplex *) cp2,(cufftComplex *) cp2,CUFFT_INVERSE);
	...
	decimate_and_redigitize<<<gridsize,blocksize>>>(dfbuf,ndec,mchan,mblock,msum,zavg,zstd,3.0,5.0,dcbuf);      
      ...
      // Copy buffer to host
      cudaMemcpy(cbuf,dcbuf,sizeof(unsigned char)*msamp*mchan/ndec,cudaMemcpyDeviceToHost);
