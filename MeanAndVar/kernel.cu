...
int main() 
{
   ...
   // const int arraySize = (1 << 20) - 331; // Set your array size
    const int nRows = 128;
    const int nCols = (1 << 20) / nRows;
    const int arraySize = nRows * nCols; // Set your array size
    const int threadsPerBlock = 1024;// BLOCK_SIZE;
   ...
   // std::cout << "Variance: " << h_variance << std::endl;
    ...
    for (int i = 0; i < num; ++i)
    {
        ...
        calcRowMeanAndDisp << < blocks, ithreads, 3 * ithreads * sizeof(float) >> >
            (d_arr, nRows, nCols, d_arrSumMean, d_arrSumMeanSquared);
        cudaDeviceSynchronize();
        kernel_OneSM_Mean_and_Disp<<<1, 1024, 1024 *3 * sizeof(float) >> >(d_arrSumMean, d_arrSumMeanSquared, nRows
            , d_sum, d_SquaredSum);
        cudaDeviceSynchronize();
...
///////////////////////////////////////////////////////////////
   ...
   //{
    ...
    duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
    std::cout << "6 Time : " << duration.count()/1000. /*/ ((double)num)*/ << " microseconds" << std::endl;
