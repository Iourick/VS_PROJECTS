...
int main(int argc, char** argv)
{
	...
	//	// F = ivctShape[0]
	//float deltaF = (vctfmin_max[1] - vctfmin_max[0]) / ((float)(ivctShape[0]));
	//int ideltaT = int(ceil((IMaxDT - 1.) * (1. / (vctfmin_max[0] * vctfmin_max[0])
	//	- 1. / ((vctfmin_max[0] + deltaF)
	//		* (vctfmin_max[0] + deltaF))) / (1. / (vctfmin_max[0]
	//			* vctfmin_max[0]) - 1. / (vctfmin_max[1] * vctfmin_max[1]))));
