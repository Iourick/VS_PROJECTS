...
{
	float deltaF = (VAlFmax - VAlFmin) / ((float)(iImgrows));
	int ideltaT = int(ceil((IMaxDT - 1.) * (1. / (VAlFmin * VAlFmax)
		- 1. / ((VAlFmin + deltaF)
			* (VAlFmin + deltaF))) / (1. / (VAlFmin
	...
	,const int IDeltaT,int* d_piarrOut)
{
		...
		for (int iF =0; iF < IImgrows; ++iF)
	{
			...
			int* d_arg1 = &d_piarrImg[iF *  IImgcols + IImgcols - i_dT];
			sumArrays << <numberOfBlocks, threadsPerBlock >> > (d_result, d_arg0, d_arg1, i_dT);
	}	
}
//-----------------------------------------------------------------------------
...
void fncFdmtIteration(int* d_piarrInp, const int IDim0, const int IDim1
	...
	, int* d_piarrOut, int & iOutPutDim0, int& iOutPutDim1, int& iOutPutDim2)
{
