...
// 1. initiate pointer to input image
	...
	for (int i = 0; i < 10; ++i)
	{
		    fncFdmt_cu_v2_(piarrImage  // input image
			, d_piarrImage
	...
	std::cout << "Time taken by function fncFdmt_cu_v0: " << duration1.count() / 10. << " milliseconds" << std::endl;
	//// !9
	double valMaxGain = duration1.count() / 10. - duration.count() / 10.;
	std::cout << "Upper bound for max gained time, maxt <=: " << valMaxGain << " milliseconds" << std::endl;
