...
{
	int n = 4;
	int* iarrin = (int*)malloc(n * sizeof(int));
	int* d_iarrin = 0;
	cudaMalloc(&d_iarrin, n * sizeof(int));
	int* iarrin1 = (int*)malloc(n * sizeof(int));
	int* d_iarrin1 = 0;
	cudaMalloc(&d_iarrin1, n * sizeof(int));

	int* iarrout = (int*)malloc(n * sizeof(int));
	int* d_iarrout = 0;
	cudaMalloc(&d_iarrout, n * sizeof(int));
	...
	cudaMemcpy(d_iarrin, iarrin, n * sizeof(int), cudaMemcpyHostToDevice);	
	cudaMemcpy(d_iarrin1, iarrin1, n * sizeof(int), cudaMemcpyHostToDevice);	


	int threadsPerBlock = 1024;
	int numberOfBlocks = (n + threadsPerBlock - 1) / threadsPerBlock;
	sumArrays << <numberOfBlocks, threadsPerBlock >> > (d_iarrout, d_iarrin1, d_iarrin,n);
	cudaDeviceSynchronize();

	cudaMemcpy(iarrout, d_iarrout, n  * sizeof(int), cudaMemcpyDeviceToHost);
